...
static void read_mrsg_config_file (const char* file_name)
{
   ...
   /*
    ...
    else if ( strcmp (property, "mrsg_map_output") == 0 )
	{
	    fscanf (file, "%d", &config_mrsg.mrsg_map_output);
	}
	else
	{
        std::cout << "Error: Property " << property << " on the config file is not valid." <<  std::endl;
        fclose(file);
	    _Exit(1);
	}
    }
